...
There is no `_NEXT` to mark consumed *for this thread* (it had none) — skip the boot-coupled stamp. **But run the supersede check (`handoff.md` step-2 "Supersede stamp"):** if the work this thread completed renders any *other* live `_NEXT_MMM` done or moot, retire that slot with `<kit-dir>/bin/next-consume.sh <system-dir> MMM "done: superseded by this fold-in"` plus a one-line CHANGELOG note. Judgment-gated — stamp only a slot you can name as genuinely done; else `next-live.sh`'s DONE-PROBABLE flag is the backstop.
