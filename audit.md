...
1. Read `<system-dir>/ledger/audit-state.md` — get `last_audit_next` and `last_audit_date`.
...
- `<system-dir>/ledger/CHANGELOG.md` — entries since `last_audit_date`
...
Write back to `<system-dir>/ledger/audit-state.md`:
```
