...
- **RE-APPLY `docs/start.html` (076 + 077)** — lifted from the `_ARCHIVE/start.html.MINE-banners-*` snapshot: Step 3 now states `setup.sh` wires `settings.json`; the broken `<system-dir>` one-liner is removed as a step and demoted to a collapsed `<details>` fallback with the token corrected to `<kit-dir>`; verify step → Step 4 `bash bin/doctor.sh`; five steps → four. Walk wording → bare **svaha** ("just type svaha" / "You svaha to act"). Verified: renders clean (4 steps, fallback, wording all live); the corrected fallback emits valid wired JSON with 0 leftover placeholders.
- **074 — `docs/start.html` Step 1** — trimmed the redundant prereq note (it duplicated the top meta line + the footer back-pointer).
- **075a — `docs/start.html` troubleshoot** — both `<system-dir>` references → `<kit-dir>` (post-split the snippet's hook paths are all `<kit-dir>`); the stale "re-do Step 4" pointer → "re-run `./setup.sh` (or the manual fallback under Step 3)".
- **075b — `README.md`** — Track B rewritten from the old manual scaffold to **`setup.sh` (machinery) + `/init` (per-project data layer)**, matching the shipped `/init`. Lines 15 & 47 repointed from the `docs/INSTALL.html` redirect stub → `docs/start.html` (resolves the FLAGGED-not-touched item in the install-friction entry below).
- **073.1 — `bin/next-write.sh`** — collision guard mirrored from `aakash_os`: `while [[ -e ]]` → atomic O_EXCL reserve (`until ( set -o noclobber; : > "$out" )`), closing the simultaneous-mint TOCTOU the sequential guard left open. `bash -n` clean; concurrent-reserve smoke test passed (two racing mints → distinct slots, none clobbered).
- **Gated on the user, NOT done here:** the `git push` to `88k8sh/svaha`; the 078 site-banner reconcile (adopt / merge / drop the `_ARCHIVE` banner polish); cold-install QA (075.2).

## 2026-06-24 — Proceed-gate word fixed to bare **svaha** (the action), not "say go" / "say svaha"

Two wording fixes to the assent ritual, for consistency across doctrine + site. **(1)** The gate-prompt was leaking generic affirmatives ("…say go to apply it") because the doctrine documented the *user's* freedom to answer with any affirmative but never pinned what word the *assistant* names when it phrases the gate. Added the rule (`CLAUDE.md` "Svaha — the assent word"): the assistant always names **svaha**, fit to the sentence ("svaha to apply it"), never a substitute. **(2)** svaha is an *action*, not something you "say" — so "say svaha" / "just say svaha" became bare **svaha** across the walkthrough (`docs/start.html`), `init.md`, and `README.md` (e.g. "start Claude Code and **svaha**"; "You **svaha** to act; it says the one seal back to confirm"). The rule now also rejects the "say svaha" wrapping. Site walkthrough + kit docs aligned; the live `~/Claude` doctrine carries the same rule.

## 2026-06-24 — Install one-liner de-frictioned: setup.sh owns settings.json end-to-end

The "one fiddly step" — a manual `sed` pipeline to write `settings.json` — was both friction **and a live bug**. Post kit-dir/system-dir split, the snippet's path placeholders are all `<kit-dir>` (8 of them), but `docs/start.html`'s manual one-liner still substituted `<system-dir>` (the comment-only occurrence, stripped before substitution). Result: structurally-valid JSON that printed "OK — written and valid" while leaving every hook command path a literal `<kit-dir>` → **every guard silently never fired.** `setup.sh` already substitutes `<kit-dir>` correctly and safe-merges an existing config (`merge-settings.py`), so the manual path was a redundant, drifted relic. The README (line 56) already described setup.sh's behavior correctly; `start.html` was the stale copy.

- **Root cause:** docs drifted from the snippet at the kit-dir/system-dir split; `setup.sh`'s fresh-write prompt also defaulted to **No**, so users `return`'d past it and were forced onto the broken manual step.
- **EDIT** `setup.sh` — fresh-write prompt `[y/N]` → `[Y/n]` (default yes; nothing to clobber when absent; mirrors the merge branch). Now setup.sh owns `settings.json` end-to-end by default. Safety contract comment updated to match. `bash -n` clean.
- **EDIT** `docs/start.html` — Step 3 now states setup.sh wires `settings.json` (write or safe-merge). The broken manual pipeline is **removed as a step** and demoted to a collapsed `<details>` fallback with the token corrected to `<kit-dir>` → `$(pwd)`. Verify step (was Step 5 `ls`) → **Step 4 `bash bin/doctor.sh`** (it actually checks settings.json validity + leftover placeholder — the exact failure the old pipeline produced). Five steps → four; secnote updated. Verified: corrected fallback emits valid JSON, 0 leftover placeholders, real absolute paths.
- **EDIT** `SYNC_MAP.md` row 51 — stale `docs/INSTALL.html (Step 3A)` ref → `docs/start.html` (now the hand-wire *fallback*); added the token-drift warning.
- **FLAGGED for the Track-B move:** `README.md` lines 15 & 47 still point install traffic at `docs/INSTALL.html` (a redirect stub) and frame the one-paste as the path — left for the queued README Track-B doc-rewrite move to avoid colliding with the concurrent docs consolidation. *(Resolved 2026-06-24 in the "Consolidate sessions 073–077" entry above — both lines repointed to `docs/start.html`.)*
