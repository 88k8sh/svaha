...
| `<system-dir>/ledger/EXTERNAL_DELIVERABLES.md` | Out-of-tree supersession registry — completions shipped outside `<system-dir>`; read FIRST by `boot.md` Step B | reference |
...
**`next:` line rules:**
- Source from open moves in `_NEXT_NNN.md` (takes priority over §6 on-demand edges). Pick the first move that verified `still-true`; skip `already-done`, surface `stale` or `superseded` (flag, never run) rather than running it.
